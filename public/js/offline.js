String.prototype.isJSON = function () {
  ...
  return (
    ...
    (a === "[" && b === "]") ||
    (a === "{" && b === "}") ||
    (a.charCodeAt(0) > 47 && b.charCodeAt(0) < 57)
  );
};
...
String.prototype.parseComponent = function (tags) {
  ...
  for (var key in tags) {
    ...
    if (tagindex === -1) tagindex = tagbeg.length - 1;
    ...
    if (beg !== -1) {
      ...
      if (end !== -1) {
        ...
        html = html
          .replace(html.substring(beg, end + tagend.length), "")
          .trim();
...
COMPONENT("websocket_offline", function (self) {
  var empty = {
  ...
  self.load = function (val) {
    ...
    W.common.components =
      val.repository ||
      "https://cdn.totaljs.com/flowstream/webcomponents/db.json";
    self.send({ TYPE: "flow" });
  };
