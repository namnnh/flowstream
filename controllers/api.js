exports.install = function () {
  ...
  // Company
  ROUTE("+API     ?    +company_create                   --> Company/create");
  ROUTE("+API     ?    +companies                        --> Company/list");

  //Site
  ROUTE("+API     ?    +sites                            --> Site/list");
  ROUTE("+API     ?    +sites_create                     --> Site/create");
...
function privatefiles($) {
  ...
  if (BLOCKED($, 10, "15 minutes")) return;
  ...
  if (filename) {
    ...
    F.Fs.lstat(path, function (err, stat) {
      ...
      $.nocache();
      $.stream(stream, U.contentTypes[U.getExtension(path)], filename, {
        "x-size": stat.size,
